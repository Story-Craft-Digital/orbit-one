...
function Component({ attributes, onSubjectClick }) {
	const {
		blockId,
		formId,
		formSource,
	...
	const currentArrowColor = resolveColor(subjectDropdownArrowColor);
	...
	const successAnimation = () => {
		...
		// Native browser WAAPI matching your exact GSAP ease/scale transitions cleanly
		msg.animate(
			[
			...
			{
				duration: 600,
				easing: "cubic-bezier(0.175, 0.885, 0.32, 1.275)", // Back ease out fallback equivalent
				fill: "forwards",
			},
		);
	};
	...
	const handleSubmit = async (event) => {
		...
		// --- 2. UI PREPARATION ---
		const formWrapper = form.closest(
			".oone-smart-contact-form-block-wrapper",
		);
		...
		// --- 3. DATA PREPARATION ---
		const formData = {
			blockName: "Orbit One Smart Contact Us Form",
			entryId: entryId,
		...
		try {
			...
			ajaxPayload.append("_ajax_nonce", window.ooneAjaxBridge?.publicNonce || "");
			...
			// Append fields dynamically into array-notated formats for proper PHP processing
		...
		if (googleFormLink && googleFormLink.includes("formResponse")) {
			...
			// Define your map of (Attribute Field Name) => (Form Value)
			const fieldMap = {
			...
			// Safely loop and append ONLY if the fieldName is a valid Google "entry." string
			Object.entries(fieldMap).forEach(([fieldName, value]) => {
				if (fieldName && fieldName.startsWith("entry.")) {
	...
	useEffect(() => {
		...
		const wrapper = document
			.getElementById(`oone-smart-contact-form-block-main-div${blockId}`)
			?.closest(".oone-smart-contact-form-block-wrapper");
...
`;
	...
	useEffect(() => {
		if (formRef.current) {
			// Calculate half the form height
			const halfHeight = formRef.current.offsetHeight / 2;
			setMaxDropdownHeight(`${halfHeight}px`);
		}
	}, [isDropdownOpen]); // Re-calculate when opened
	return (
		<div
			id={`oone-smart-contact-form-block-main-div${blockId}`}
			style={{
				display: "flex",
				width: "100%",
				minHeight: "100%",
		...
		>
			...
			>
				<form
					ref={formRef}
					className="oone-smart-contact-form-block storycraft-contact-form storycraft-form"
					id={`oone-smart-contact-form-block-form${formId}`}
					style={{
						width: "100%",
					...
					>
						...
						>
							...
							>
								...
								>
									{/* The Main Dropdown Trigger */}
									<div
										onClick={() => {
											setIsDropdownOpen(!isDropdownOpen);
											// ✅ If we are in the editor and the prop exists, fire it!
											if (onSubjectClick) {
												onSubjectClick();
											}
										}}
										style={{
											...
											border: errors.subject
												? "2px solid red"
												: `${inputBorder}px solid ${
														isDropdownOpen
															? resolveColor(inputBorderFocusColor)
															: resolveColor(inputBorderColor, "#ccc")
												  }`,
											fontSize: `${calculatedPlaceholderSize}px`,
											transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
											boxShadow:
												isDropdownOpen && inputShadow.enabled
													? resolvedShadow
													: "none",
										}}
									>
										<span
											style={{
												color: selectedSubject ? "#333" : "#7c7a7a",
												whiteSpace: "nowrap",
												overflow: "hidden",
												textOverflow: "ellipsis",
												fontSize: selectedSubject
													? `${calculatedPlaceholderSize}px`
													: `${calculatedPlaceholderSize}px`,
											}}
										>
									...
									{isDropdownOpen && (
										<div
											style={{
												...
												border: `1px solid ${resolveColor(inputBorderColor)}22`,
												...
												scrollbarColor: `${resolveColor(
													titleColor,
												)} transparent`,
											}}
											className="oone-custom-scrollbar" // Add a class for the Webkit styles
										>
											{/* ✅ INJECT CUSTOM SCROLLBAR CSS */}
											<style>{`
            .oone-custom-scrollbar::-webkit-scrollbar {
                width: 6px;
            }
            .oone-custom-scrollbar::-webkit-scrollbar-track {
                background: transparent;
            }
            .oone-custom-scrollbar::-webkit-scrollbar-thumb {
                background-color: ${resolveColor(titleColor)};
                border-radius: 10px;
            }
        ...
        `}</style>
														...
														style={{
															...
															background:
																selectedSubject === subject
																	? `${resolveColor(backgroundColor)}11`
																	: "transparent",
															color:
																selectedSubject === subject
								...
								{/* HIDDEN INPUT: This keeps your handleSubmit function working exactly as before */}
								<input
									type="hidden"
									id="subject"
...
document.addEventListener("DOMContentLoaded", function () {
	document
		.querySelectorAll(".oone-smart-contact-form-block-wrapper")
		.forEach((wrapper) => {
			...
			const rootElement = wrapper.querySelector(
				`#oone-smart-contact-form-block-root-component-${blockId}`,
			);
