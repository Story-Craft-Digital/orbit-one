 ...
 * @param {object} props
 * @param {function(string):void} props.onSelect Callback function that receives the *key* (e.g., "primary").
 */
	...
	>
		...
		>
			{["top", "right", "bottom", "left"].map((side) => (
				<div key={side}>
					<TextControl
						label={__(side.toUpperCase(), "oone-addon-carousel")}
						value={values?.[side] || ""}
						type="number"
...
export default function Edit({ attributes, setAttributes }) {
	...
	/**
	 * ✅ UPDATED: Create gradient objects with both resolved CSS (for display)
	 * and key-based strings (for saving).
	 */
	const paletteGradients = useMemo(() => {
		const oonePalette = window.oonePalette || {};
		if (isLoading || !oonePalette.primary) return []; // Wait for loading & palette
	...
	useEffect(() => {
		const initializeBlock = () => {
			...
			if (slides.length > 0 && !slides[0].id) {
				initialUpdates.slides = slides.map((slide) => ({
					...slide,
		...
		setAttributes({
			slides: [
				...slides,
				{
					...
					link: "#",
					image:
						"oone-brand-carousel-image-1.webp",
					imgAlt: "",
				},
			],
		});
							...
							<>
								{tab.name === "typography" && (
									<div style={{ marginTop: "20px", marginBottom: "20px" }}>
										...
										{/* Caption Line Height */}
										<RangeControl
											label={__("Caption Line Height", "orbit-one")}
										...
										{/* Title One */}
										<TextControl
											label={__("Title One", "orbit-one")}
											value={titleOne || ""}
											onChange={(value) => setAttributes({ titleOne: value })}
										/>
										{/* Title Two */}
										<TextControl
											label={__("Title Two", "orbit-one")}
											value={titleTwo || ""}
								...
								{tab.name === "color" && (
									<div style={{ marginTop: "20px", marginBottom: "20px" }}>
																...
																{tab.name === "gradient" && (
																	<div style={{ marginTop: "10px" }}>
																		<GradientPicker
																			...
																			/**
																			 * ✅ UPDATED onChange
																			 * When a gradient is picked, check if it's a preset.
																			 * If yes, save the 'keyString'.
																			 * If no (it's custom), save the raw CSS.
																			 */
																			onChange={(gradientCSS) => {
																				...
																				// Find the preset that matches this resolved CSS
																				const preset = paletteGradients.find(
																					(p) => p.gradient === gradientCSS,
																				);

																				if (preset) {
																					// Found a preset! Save the keyString.
																					setAttributes({
																						backgroundColor: preset.keyString,
																					});
																				} else {
																					// It's a custom gradient. Save the raw CSS.
																					setAttributes({
										...
										<div style={{ marginBottom: "15px" }}>
											...
											<Button
												onClick={() => toggleColorPicker("slidesColor")}
												style={{
													background: resolveColor(slidesColor) || "#333",
													color: "#FFFFFF",
													textShadow: "1px 1px 2px rgba(0, 0, 0, 0.9)",
													border: "1px solid #ddd",
													padding: "8px 16px",
															...
															<>
																{tab.name === "color" && (
																	<div style={{ marginTop: "10px" }}>
																		<PaletteButtons
																			onSelect={(key) =>
																				setAttributes({ slidesColor: key })
																			}
																		/>
																		<ColorPicker
																			color={resolveColor(slidesColor)}
																			onChangeComplete={(color) =>
																				setAttributes({
																					slidesColor: color.hex,
																				})
																			}
																			disableAlpha
																		/>
																...
																{tab.name === "gradient" && (
																	<div style={{ marginTop: "10px" }}>
																		<GradientPicker
																			value={resolveColor(slidesColor)}
																			/**
																			 * ✅ UPDATED onChange
																			 */
																			onChange={(gradientCSS) => {
																				if (!gradientCSS) {
																					setAttributes({ slidesColor: "" });
																					return;
																				}
																				const preset = paletteGradients.find(
																					(p) => p.gradient === gradientCSS,
																				...
																				if (preset) {
																					setAttributes({
																						slidesColor: preset.keyString,
																					});
																				} else {
																					setAttributes({
																						slidesColor: gradientCSS,
																					});
																				}
																			}}
																			gradients={[...paletteGradients]}
