...
export default function Edit({ attributes, setAttributes }) {
	...
	const fetchFormData = async () => {
		...
		try {
			// Catch uninitialized bridge context errors early
			...
			// Format parameters into native FormData loop layers
			...
			const subjectEntry = result.entries.find((entry) =>
				/Subject|Feedback Topic|Reason/i.test(entry.label),
			);
			if (subjectEntry) {
				newAttributes.subjectFieldName = subjectEntry.name;
			}
			...
			// Fallback mappings matching exact expression evaluations
	...
	return (
		<div {...useBlockProps({ className: "storycraft-contact-form" })}>
			...
			<InspectorControls>
				...
				{/* 🎨 CONTENT PANEL */}
				<PanelBody title={__("Content", "orbit-one")} initialOpen={false}>
					<TabPanel
						...
						{(tab) => (
							<>
								{tab.name === "fieldNames" && (
									...
									>
										{/* THE MAIN SUBJECT INPUT */}
										<TextControl
											label={__("Form Subject", "orbit-one")}
											value={subject} // ✅ Bind directly to the 'subject' attribute
											onChange={(value) => setAttributes({ subject: value })}
											help={__(
												"The default subject sent with the form submission.",
											)}
											placeholder={__("e.g. Feedback")}
										/>
