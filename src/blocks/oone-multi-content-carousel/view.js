...
/**
 * ✅ NEW: Helper to convert a palette key or hex string to an RGBA value.
 * This replaces the old `getValidColorForFade`.
 */
const resolveToRgba = (colorValue, opacity) => {
	// First, get the final color (e.g., "accent" -> "#2e1065")
	let hex = resolveColor(colorValue, "#000000");

	// If it's a gradient, transparent, or invalid, fallback to a transparent black
	if (
...
const getBorderRadiusString = (obj, scale = 1) => {
	if (!obj) return `${8 * scale}px`; // Default radius 8px, scaled
	return `${(obj.topLeft || 0) * scale}px ${(obj.topRight || 0) * scale}px ${
		(obj.bottomRight || 0) * scale
	}px ${(obj.bottomLeft || 0) * scale}px`;
};
...
const Slide = ({
	...
	const gradientStyle =
		item.textPosition === "top"
			? `linear-gradient(to bottom, ${resolveToRgba(
					finalFilterColor,
					0.8,
			  )}, transparent)`
			: `linear-gradient(to top, ${resolveToRgba(
					finalFilterColor,
					0.8,
			  )}, transparent)`;
...
function Component({ attributes, useEditor = false, onSlideClick = () => {} }) {
	...
	useEffect(() => {
		...
		const blockSlug = "oone-multi-content-carousel";
	...
	// --- UTILITY FUNCTIONS ---
	const getValidColor = (color) => color || "transparent";
	const getValidColorForFade = (color) => {
		...
		if (color.startsWith("rgb"))
			return color.replace(/rgb\(([^)]+)\)/, "rgba($1, 0.8)");
		return "rgba(0,0,0,0)";
	};
						...
						<li key={item.id || index} style={{ display: "inline-block" }}>
							<Slide
								...
								additionalStyles={{
									marginLeft:
										index === 0
											? `${(titlePadding.left || 0) * dimensions.fontScale}px`
											: "0px",
									marginRight:
										index === slides.length - 1
											? `${(titlePadding.right || 0) * dimensions.fontScale}px`
											: "0px",
								}}
					...
					>
						<div
							style={{
								width: `${progress}%`,
								height: "100%",
								background: resolveColor(progressbarColor, "#000"),
								borderRadius: `${4 * dimensions.fontScale}px`,
...
if (typeof window !== "undefined") {
	document.addEventListener("DOMContentLoaded", function () {
		document
			.querySelectorAll(".oone-multi-content-carousel-block-wrapper")
			.forEach((wrapper) => {
				...
				const rootElement = wrapper.querySelector(
					`#oone-multi-content-carousel-block-root-component-${blockId}`,
				);
